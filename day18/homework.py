...
#
# find the largest magnitude from adding 2 numbers
#
def largestMagnitude (numbers):
    largestMagnitude = 0
    largestFirstNumber = None
    largestSecondNumber = None
    largestResult = None
    ...
    for f in numbers:
        ...
        for s in numbers:
            if f != s:
                sCopy = copy.copy(s)
                sumNumber = addNumbers([fCopy,sCopy])
                magnitude = sumNumber.magnitude()
                if magnitude > largestMagnitude:
                    largestMagnitude = magnitude
                    largestFirstNumber = f
                    largestSecondNumber = s
                    largestResult = copy.copy(sumNumber)

    return largestMagnitude, largestFirstNumber, largestSecondNumber, largestResult
...
def main():
    ...
    # Do Part 2 work
    print()
    magnitude, firstNumber, secondNumber, result = largestMagnitude(numbers)
    print()
    print("{}Largest Magnitude: {}{}{}".format(color.CYAN, color.YELLOW, magnitude, color.END))
    print()
    print(" ",firstNumber)
    print("+",secondNumber)
    print("=",result)
